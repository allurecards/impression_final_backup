...
const CATEGORY_ORDER = Object.keys(TEMPLATE_CATEGORY_LABELS) as Array<
...
export function TemplatePicker() {
  ...
  const filtered = useMemo(
    () =>
      activeCategory === "all"
        ? TEMPLATES
        : TEMPLATES.filter((t) => t.categories.includes(activeCategory as never)),
    [activeCategory],
  );
        ...
        {filtered.map((t) => (
          ...
          >
            {t.type === "image" && t.imageSrc ? (
              ...
              >
                <span
                  className="h-9 w-7 rounded-sm"
                  style={{ backgroundColor: t.panel, border: `1px solid ${t.border}88` }}
                />
              </span>
            )}
