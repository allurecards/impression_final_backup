    ...
    <Group label="Background">
      <label className="block text-sm">
        ...
        <div className="flex items-center gap-2">
          ...
          <button type="button" onClick={() => setField("imageBg", DEFAULT_DESIGN.imageBg)} className="ml-auto text-xs underline opacity-70">
            Reset
          </button>
        </div>
      </label>
      <div className="mt-4">
        <Slider label="Text Y offset" min={-100} max={100} value={state.textOffset} onChange={(v) => setField("textOffset", v)} />
      </div>
    </Group>
  );
}
